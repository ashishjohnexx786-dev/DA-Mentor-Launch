(()=>{
  ...
  function applyData(){if(typeof C!=="undefined")C.forEach(p=>p.plan.forEach(u=>u.videoStatus=status(u.id)));}
  function decorate(){
    const p=typeof cur==="function"?cur():null;if(!p)return;
    document.querySelectorAll("#schedule .item").forEach((row,i)=>{
      row.querySelector(".courseVideoStatus")?.remove();
      const u=p.plan[i];if(!u)return;
  ...
  function install(){
    ...
    if(typeof render==="function"){
      const base=render;render=function(){base();decorate()};render();
    }
